var fs = require("fs"), path = require("path"), util = require("util"),
    events = require("events"), async = require("async"),
...
function DirectoryReader(dirs, excludeDirs) {
  ...
  process.nextTick(function() {
    try {
      if (typeof self.excludeDirs === "string" || self.excludeDirs instanceof
                                                      String) {
      ...
      for (var i = 0; i < self.excludeDirs.length; ++i) {
        ...
        if (!stat.isDirectory()) {
          throw new Error(
              `Path ${self.excludeDirs[i]} is not a directory`,
          );
        }
      }
    } catch (error) {
...
DirectoryReader.prototype._pushDir = function(nextCB, errorCB) {
  var self = this, dir = self.currentFilePath;
...
DirectoryReader.prototype._isExcludedDir = function() {
  var excludeDirs = this.excludeDirs, dir = path.resolve(this.currentFilePath);
...
DirectoryReader.prototype.list = function() {
  ...
  self._pushDir(function() { self.next(); });
};
